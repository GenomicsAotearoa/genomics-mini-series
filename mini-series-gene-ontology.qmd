...
You need to think carefully about what you want to use as your background gene list. Generating it is fairly straight forward, but it has implications for the analysis. Read more on how overrepresentation analysis works and how the background gene list is used in the analysis [here in the functional analysis section of the RNA-seq workshop](https://genomicsaotearoa.github.io/RNA-seq-data-analysis-workflow/episode_5.html) and in this paper on ["Ten common mistakes that could ruin your enrichment analysis"](https://doi.org/10.1371/journal.pcbi.1014122) by Bora, McKenzie and Ziemann, 2026.  All genes that you are testing (*i.e.,* all your DEGs) must be in the background gene list. The simplest option for generating this list is to use all genes that are *expressed* in your dataset as the background. 

We will take these gene IDs by pulling all the rownames out of the `dds` object:
...
If your data is a text file, you should read it into R as a dataframe. The first row should be the header.  If you use `read.delim` from `utils` you can specify header is true (or false if you have no header) and the separator is tab (or alternate delimiter).
...
::: {.callout-note collapse="true"}
# How to process the `.gaf` file...

There are many ways in shell or R you could process this file. You will need to:

1. Remove the header lines
2. Pull out the Symbol column and GO_ID column 
3. Add any missing gene IDs that are not in the gaf file, but are in your counts file (any genes that can be mapped to, but do not have GO annotations).
4. Collapse GO IDs into a single string for each gene ID. 

Here is one way you could acheive this. 

In shell to start:

```bash
# download your file with wget
wget "https://ftp.ncbi.nlm.nih.gov/genomes/all/annotation_releases/path/to/gene_ontology.gaf.gz"

# unzip
gunzip *.gaf.gz

# remove header
cat gene_ontology.gaf | grep -v '^!' > noheadergaf.txt

# grab only the third and fifth columns (Symbol and GO_ID) 
awk -F'\t' '{print $3"\t"$5}' noheadergaf.txt > symbol_GOIDs.txt
```
...
# read in data
GO_data_raw <- read_tsv("symbol_GOIDs.txt", col_names = c("Symbol", "GO_ID"))

# grab just the Gene IDs column from your counts object - this is a single column dataframe with one row for every mappable gene 
all_genes <- as.data.frame(counts[,1]) 
colnames(all_genes) <-  "Symbol" #rename column to symbol to match the data_raw

# Join together your dataframe that has ALL the gene IDs, with the dataframe that has gene IDs matched to GOs. 
# Not all genes will have a GO ID, but we need them in the final dataframe with "NA" in the GO_ID column. 
all_genes_GOs <- left_join(all_genes, GO_data_raw, by = "Symbol") 

# Check - this number should be the same number as how many genes are in your raw counts matrix
length(unique(all_genes_GOs$Symbol))

# Finally, collapse all GO_IDs for one gene into a single string
geneOntologies <- all_genes_GOs %>%
  group_by(Symbol) %>%
  summarise(GO_ID = if (all(is.na(GO_ID))) NA_character_ 
            else paste(unique(na.omit(GO_ID)), collapse = ",")) |> 
  as.data.frame()

# Final checks - the number of genes in your final geneOntologies object should be the same number of genes in your counts and the same number of unique genes in your uncollapsed dataframe

nrow(geneOntologies) == length(unique(all_genes_GOs$Symbol))
# should print TRUE

nrow(geneOntologies) == nrow(counts)
# should print TRUE

```

> Note: Don't worry about duplicate GO terms in the final collapsed string for a gene. GOseq removes duplicates during the `goseq(... gene2cat = splitgoannot)` step. The `enricher()` function also removes duplicates during the `enricher(... TERM2GENE = term2gene)` step. 


:::


***Option 2 (harder):*** Derive GOs from BLAST and Pfam annotations. This is more work, but may be necessary if you don't have a `.gaf` file available for your organism. You can use tools like `InterProScan` to annotate your genes with GO terms based on their sequence similarity to known proteins. BLAST2GO is another tool (commercial - need a license) that can be used to assign GO terms to genes based on BLAST results (*i.e.,* BLASTing against organisms that *do* have GO annotations, usually from the evolutionarily nearest model organism!). You can do this manually without BLAST2GO too. You would need to BLAST your genes against a database of genes with known GO annotations, and then use the results to assign GO terms to your genes based on the best hits. If you generate Pfam annotations through another pipeline (*e.g.,* through hmmer) you can download the latest [PFAM2GO mapping file](https://current.geneontology.org/ontology/external2go/pfam2go) to retrieve GO terms for your genes based on their Pfam domains. These approaches are more time-consuming, but are a useful alternative for non-model organisms or for less annotated genomes. 
*De novo* RNAseq methods frequently use the [Trinity](https://github.com/trinityrnaseq/trinityrnaseq/wiki) - [Trinotate](https://github.com/Trinotate/Trinotate/wiki) pipeline to assemble and annotate (unfortunately, as of March 2024 there is no longer ongoing development and support for these tools.) 
...
# last two tests - DE.vector should have the same number of genes as the length.vector now and the gene names should be in the same order in both. 
# If you do not get two TRUEs - go back and debug. 
...
goout_MT_UP_over |> 
  ...
  labs(title="Over-represented GO terms for genes up in MT, vs F",x = "GO Term", y = "GeneRatio", fill = "-log10(p-value)") + 
  theme_classic()
```
...
1. Find orthologues of your species genes to a related species that does have an OrgDb. Once you have matched one-to-one the gene IDs, you can use the OrgDb species gene IDs in place of your own gene IDs and run the standard goseq pipeline using the supported organism. 

Caveats:  

- Gene lengths may not be accurate   
- You lose genes with no orthologues, less of a problem for a very closely related species.  

You can also use your own gene lengths, but use the annotations from the supported organism. 
...
# Standard pipeline but with custom gene lengths
# Use case: your gene IDs match a supported org, but you want your own lengths

library(goseq)
library(org.Sc.sgd.db)

data.pwf <- nullp(DEgenes  = DE.vector,        # binary named vector using supported org gene IDs
                  bias.data = length.vector)   # your custom lengths

go_output <- goseq(pwf    = data.pwf, 
                   genome = "sacCer1",          # supported org genome
                   id     = "ensGene",          # supported org gene ID type
                   gene2cat = NULL)             # NULL = fetch GO terms automatically
``` 

2. Use `biomaRt` to find orthologues, which can query Ensembl for ortholog mappings between species. 
